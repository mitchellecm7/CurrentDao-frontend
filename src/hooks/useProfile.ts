...
const mockProfile: UserProfile = {
  ...
  level: "advanced",
  // Name service fields
  stellarAddress: "GD4TQ7V3QBXZ2X4W4OZ6XGJZ7Z2Y6XGJZ7Z2Y6XGJZ7Z2Y6XGJZ7Z2Y",
  snsName: "johndoe.sns",
  federatedAddress: "johndoe*currentdao.io",
  displayName: "johndoe.sns",
...
export function useProfile(): ProfileContextType {
  ...
  const updateAccountSettings = useCallback(
    async (settings: Partial<AccountSettings>) => {
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          accountSettings: prev.accountSettings
            ? { ...prev.accountSettings, ...settings }
            : null,
        }));
  ...
  const updateTradingPreferences = useCallback(
    async (preferences: Partial<TradingPreferences>) => {
      ...
      try {
        ...
        if (
          ...
          (preferences.locationRadius < 1 || preferences.locationRadius > 1000)
        ) {
          throw new Error("Location radius must be between 1 and 1000 km");
        }

        if (
          preferences.minimumRating !== undefined &&
          (preferences.minimumRating < 0 || preferences.minimumRating > 5)
        ) {
        ...
        setState((prev) => ({
          ...prev,
          tradingPreferences: prev.tradingPreferences
            ? { ...prev.tradingPreferences, ...preferences }
            : null,
        }));
      ...
      try {
        ...
        setState((prev) => ({
          ...
          securitySettings: prev.securitySettings
            ? { ...prev.securitySettings, ...settings }
            : null,
        }));
  ...
  const enableTwoFactor = useCallback(
    async (method: "sms" | "email" | "authenticator") => {
      setUpdating(true);
      setError(null);
      ...
      try {
        ...
        setState((prev) => ({
          ...
          securitySettings: prev.securitySettings
            ? {
                ...prev.securitySettings,
                activeSessions: prev.securitySettings.activeSessions.filter(
                  (s) => s.id !== sessionId,
                ),
              }
            : null,
      ...
      try {
        ...
        setState((prev) => ({
          ...
          securitySettings: prev.securitySettings
            ? {
                ...prev.securitySettings,
                apiKeys: prev.securitySettings.apiKeys.filter(
                  (k) => k.id !== keyId,
                ),
              }
            : null,
  ...
  const resolveNameService = useCallback(
    async (address: string): Promise<NameServiceResolution> => {
      try {
        return await resolveStellarName(address);
      } catch (error) {
  ...
  const updateStellarAddress = useCallback(
    async (address: string) => {
      ...
      try {
        ...
        const resolution = await resolveNameService(address);

        // Update profile with Stellar address and resolved name
        const updates: Partial<UserProfile> = {
          stellarAddress: address,
        };

        if (resolution.type !== "none" && resolution.name) {
          if (resolution.type === "sns") {
            updates.snsName = resolution.name;
            updates.displayName = resolution.name;
          } else if (resolution.type === "federated") {
            updates.federatedAddress = resolution.name;
            updates.displayName = resolution.name;
          }
        }
        ...
        updates.profileCompletion = calculateProfileCompletion(updatedProfile);
        ...
        toast.success("Stellar address updated successfully");
      } catch (error) {
        const errorMessage =
          error instanceof Error
            ? error.message
    ...
    [state.profile, resolveNameService, setUpdating, setError],
  );

  const getProfileCompletion = useCallback((): ProfileCompletionItem[] => {
    if (!state.profile) return [];
    ...
    return [
      ...
      },
      {
        ...
        completed: !!profile.stellarAddress,
        required: false,
        weight: 20,
      },
      {
  ...
  const getPublicProfileUrl = useCallback((userId: string): string => {
