...
export function TransactionQueue({ className }: TransactionQueueProps) {
  ...
  const getTransactionColor = (type: QueuedTransaction['type']) => {
    switch (type) {
      case 'send': return 'text-blue-600 bg-blue-100'
      case 'swap': return 'text-green-600 bg-green-100'
      case 'stake': return 'text-purple-600 bg-purple-100'
      case 'vote': return 'text-orange-600 bg-orange-100'
      case 'delegate': return 'text-pink-600 bg-pink-100'
  ...
  return (
    <div className={`relative ${className || ''}`} ref={queueRef}>
      {/* Queue Button */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`relative p-2 rounded-lg transition-colors ${
          pendingCount > 0 ? "bg-orange-100 text-orange-600 hover:bg-orange-200" : "hover:bg-gray-100"
        }`}
      >
      ...
      {/* Queue Dropdown */}
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-96 bg-white border rounded-lg shadow-lg z-50 max-h-96 overflow-hidden flex flex-col">
          {/* Header */}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${getTransactionColor(tx.type)}`}>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-600">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className={tx.expiresAt <= new Date() ? "text-red-600" : ""}>
