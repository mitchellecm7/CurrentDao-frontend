...
const mockSetState = <T,>(initialValue: T): [T, (value: T | ((prev: T) => T)) => void] => {
  return [initialValue, () => {}];
};
...
const mockUseRef = <T>(initialValue: T): { current: T } => {
...
const createIcon = (emoji: string) => ({ className, ...props }: any) => 
  createElement('span', { className, ...props }, emoji);
